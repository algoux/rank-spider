...
import { run, DomjudgeHtmlV10Asset } from '../adapters/domjudge_html_v10';
...
function createAssetHandler(htmlPath: string, outputPath: string) {
  ...
  return async (asset: DomjudgeHtmlV10Asset): Promise<string | undefined> => {
    ...
    if (isHttpUrl(asset.source)) {
      const res = await fetch(asset.source, {
        headers: {
          'user-agent': 'Mozilla/5.0 (compatible; rank-spider DOMjudge HTML v10 program)',
        },
      });
      if (!res.ok) {
...
program
  .name('domjudge_html_v10.ts')
  .argument('<html-file>', 'DOMjudge v10 HTML file path')
